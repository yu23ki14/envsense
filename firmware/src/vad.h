...
// Energy-based voice activity detection over 20ms PCM frames. Silence is
// dropped before the Opus encoder, which is where the always-on capture's
// power and storage savings come from. A PSRAM pre-roll ring keeps the last
// VAD_PREROLL_MS of PCM so the start of an utterance isn't clipped, and a
// VAD_HANGOVER_MS hangover bridges normal speech pauses.
