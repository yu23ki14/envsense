...
top_h_front   = 2.0;   // 前(USB側)2隅の上押さえ高さ。上方が空くので高め可
                       //   （2.5 → 2.0: board_top_gap +0.5 と同時に -0.5 し、爪天
                       //   z1 = board_top_gap + top_h_front = 3.0 の全高を維持。2026-06）
...
board_top_gap = 1.0;   // 押さえ爪を上へ平行移動する量＝基板上面との隙間（爪厚 top_h は維持）。
                       //   後隅の爪は廃止済みなので実質前(USB側)2隅のみに効く
                       //   （0.5 → 1.0: 実機で挟み隙間が狭く基板が窮屈だったため +0.5。
                       //   爪天 z=3.0 は top_h_front -0.5 で維持。2026-06）
...
sd_clr_x  = 1.0;   // カード先端(-X)の余裕（pebble の盲チャネル前面のみ）。実機でカード先端が
                   //   前壁に当たったため追加（2026-06）。box は貫通スロットなので使わない
