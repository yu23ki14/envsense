...
//   - X には盲: 前面 = USB ザグリ底(-3.1) − sd_clr_x(1.0) = -4.1。カード先端(-2.5)に
//     1.6mm の余裕（旧: ザグリ底と同一面で 0.6mm → 実機でカード先端が前壁に当たり拡大）。
//     ザグリ底より「浅い」と井戸との間に 0.1mm 薄肉が残るが、「深い」方向は段差 1mm が
//     付くだけで薄肉は出ない（チャネル上部は井戸と連結し、井戸を覗くとカード先端が
//     見えるが外面開口は増えない。プラグ樹脂はザグリ底 x=-3.1 で止まる）。
...
module cut_sd_peb() {
    ...
    x_front = -usb_overhang - usb_well_clr - sd_clr_x;   // = USB ザグリ底 - 1.0（-4.1）
